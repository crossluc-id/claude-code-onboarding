...
function detectFromLockFile(projectDir = process.cwd()) {
  for (const pmName of DETECTION_PRIORITY) {
    ...
    // Check legacy lock file (e.g., bun.lockb for older Bun versions)
    if (pm.lockFileLegacy) {
      const legacyPath = path.join(projectDir, pm.lockFileLegacy);
